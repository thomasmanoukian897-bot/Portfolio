...
<article {{ $attributes->merge(['class' => 'post-card group flex flex-col bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden hover:shadow-md transition-shadow']) }}>
    <a href="{{ route('posts.show', $post) }}" class="post-card-media block aspect-[16/9] overflow-hidden bg-slate-100 dark:bg-slate-800 relative">
        ...
        @else
            <div class="absolute inset-0 bg-gradient-to-br from-slate-100 via-blue-50/80 to-slate-200 dark:from-slate-800 dark:via-slate-800 dark:to-slate-900">
                ...
                <div class="absolute inset-0 flex items-center justify-center">
                    <x-user-avatar :user="$post->user" size="lg" class="post-card-placeholder-avatar w-24 h-24 text-3xl ring-4 ring-white/50 dark:ring-slate-700/50 shadow-lg" />
                </div>
            </div>
        @endif
    ...
    <div class="post-card-body p-6 flex flex-col">
        <div class="post-card-meta flex items-center gap-2 text-xs text-slate-500 mb-4">
        ...
        @if ($post->categories->isNotEmpty())
            <div class="post-card-categories flex flex-wrap gap-2 mb-4">
        ...
        <h2 class="post-card-title text-xl font-bold text-slate-900 dark:text-slate-100 font-display mb-3 group-hover:text-primary transition-colors">
        ...
        @if ($post->excerpt)
            <p class="post-card-excerpt text-sm text-slate-600 dark:text-slate-400 leading-relaxed mb-6 line-clamp-4">
                {{ $post->excerpt }}
            </p>
        @endif

        <div class="post-card-footer flex items-center justify-between gap-4 {{ $post->excerpt ? 'mt-2' : 'mt-4' }}">
            <a
                href="{{ route('posts.show', $post) }}"
                class="post-card-read-more inline-flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-primary hover:text-blue-700 transition-colors"
            >
                Read More
                ...
                @can('delete', $post)
                    <form method="POST" action="{{ route('posts.destroy', $post) }}" onsubmit="return confirm('Delete this post? This cannot be undone.')">
                        ...
                        <button
                            type="submit"
                            aria-label="Delete"
                            class="inline-flex items-center justify-center px-3 py-1.5 rounded-lg text-xs bg-red-50 hover:bg-red-100 dark:bg-red-950/50 dark:hover:bg-red-950 text-red-700 dark:text-red-400 border border-red-200 dark:border-red-800 transition-colors cursor-pointer"
                        >
